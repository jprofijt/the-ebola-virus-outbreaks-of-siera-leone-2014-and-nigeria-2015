...
```{r}
st.out <- as.data.frame(st.out)
round.out <- round(st.out)

plot(round.out[,1], round.out[,2],
     type = "l",
     main = "People suseptible to virus infection",
     ylab = "amount of people",
     xlab = "time in days",
     col = "red")
lines(st.out[,1], st.out[,2],
      type = "l",
      col = "black")
    
plot(round.out[,1], round.out[,3],
     type = "l",
     main = "People exposed to virus",
     ylab = "Cumulative number of cases",
     xlab = "time in days",
     col = "red")
lines(st.out[,1], st.out[,3],
      type = "l",
      col = "black")
    
plot(round.out[,1], round.out[,4],
     type = "l",
     main = "Amount of people infected with ebola virus",
     ylab = "Cumulative number of cases",
     xlab = "time in days",
     col = "red")
lines(st.out[,1], st.out[,4],
      type = "l",
      col = "black")
    
plot(round.out[,1], round.out[,5],
     type = "l",
     main = "people who have recovered from virus infection",
     ylab = "Cumulative number of cases",
     xlab = "time in days",
     col = "red")
lines(st.out[,1], st.out[,5],
      type = "l",
      col = "black")
    
plot(round.out[,1], round.out[,6],
     type = "l",
     main = "Amount of dead people",
     ylab = "Cumulative number of cases",
     xlab = "time in days",
     col = "red")
lines(st.out[,1], st.out[,6],
      type = "l",
      col = "black")
...
difference.by.day <- function(x, collumn){
  ...
  x[collumn] - day.before
  
}
difference.susceptible <- apply(round.out, 1, difference.by.day, "S")
difference.exposed <- apply(round.out, 1, difference.by.day, "E")
difference.infected <- apply(round.out, 1, difference.by.day, "I")
