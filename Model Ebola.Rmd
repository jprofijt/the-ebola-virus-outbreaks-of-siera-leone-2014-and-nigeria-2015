...
#begint met 5 doden en 6 totale cases
exp.state <- c(S = 7.396*10^6,
               E = 0,
               I = 6,
               R = 0,
               D = 5)
...
get.total.infected <- function(x){
  cases <- x[1]
  index <- subset(exp.out, Time <= cases, select = c("I", "D", "R"))
  sum(index)
}

total.cases <- apply(round(exp.out), 1, get.total.infected)

optim.cases <- function(exp, model){
  diff <- (exp[6] - model[[10]])^2
}


optim.test <- optim(ebola_data.sorted$Total.Cases..Sierra.Leone, optim.cases, model = total.cases, method = "Nelder-Mead")
plot(ebola_data.sorted$Total.Deaths..Sierra.Leone ~ ebola_data.sorted$time.points ,
     type = "p",
     pch = 20,
     col="black")
lines(exp.out$D ~ exp.out$Time, col = "red")
